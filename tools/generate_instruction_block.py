...
def svg_iso(path, az=-45, el=26, title='Bloco de Instrução 3D'):
    ...
    for _,poly,mat in sorted(pf,key=lambda a:a[0]):
        pts=' '.join(f'{ox+x*s:.1f},{oy+y*s:.1f}' for x,y in poly); opacity='.55' if mat=='glass' else '1'
...
def svg_plan(path,floor=0):
    W,H=1200,650; sx=(W-80)/BUILDING_W; sz=(H-100)/BUILDING_D
